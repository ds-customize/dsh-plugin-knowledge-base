...
function apply(ctx, config) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (enabled.length === 0) return { ...base, matches: [], filesScanned: 0, notice: "No enabled knowledge base connections." };
			const tokens = tokenizeQuery(query);
			if (tokens.length === 0) return { ...base, matches: [], filesScanned: 0, notice: "The search query contains no usable terms." };
			const notices = [];
			const ranked = [];
			let filesScanned = 0;
			let matched = 0;
